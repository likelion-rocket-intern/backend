...
from .resume import ResumeJobDescriptionLink
...
class Description(SQLModel, table=True):
    ...
    # 추가 필요: 역방향 관계 설정
    job_description: "JobDescription" = Relationship(back_populates="descriptions")
...
class JobDescriptionResult(SQLModel, table=True):
    ...
    resume_strengths: List[Dict[str, Any]] = Field(sa_column=Column(JSONB, nullable=False))
    resume_weaknesses: List[Dict[str, Any]] = Field(sa_column=Column(JSONB, nullable=False))
    overall_assessment: Dict[str, Any] = Field(sa_column=Column(JSONB, nullable=False))
